...
#4mm X 4mm grid on 15mm x 15mm base 
...
def get_mla_centres(pitch_mla, lens_number=3):
    ...
    Returns:
    centres : np.ndarray
        Array of shape (N, 2) containing the x and y coordinates of the MLA centres.
    """
    ...
    if lens_number == 3 :
        ...
        centres[0, 0] = - pitch_mla / 2# x_centre of lens 1 
        centres[0, 1] = - pitch_mla / (2 * np.sqrt(3)) # y_centre of lens 1
        # Calculate the lower right lens centre. (as the second lens in the array)
        centres[1, 0] = pitch_mla / 2 # x_centre of lens 2
        centres[1, 1] = - pitch_mla / (2 * np.sqrt(3)) # y_centre of lens 2
        # Calculate the upper lens centre. (as the third lens in the array)
        centres[2, 0] = 0 # x_centre of lens 3
        centres[2, 1] = pitch_mla / np.sqrt(3) # y_centre of lens 3
    ...
    if lens_number == 7:
        ...
        centres[0, 0] = - pitch_mla / 2
        centres[0, 1] = pitch_mla * np.sqrt(3)/2
        # Top right lens centre (2nd lens)
        centres[1, 0] = pitch_mla / 2
        centres[1, 1] = pitch_mla * np.sqrt(3)/2
        # Middle left lens centre (3rd lens)
        centres[2, 0] = - pitch_mla
        centres[2, 1] = 0
        # Centre lens (4th lens)
        centres[3, 0] = 0
        centres[3, 1] = 0
        # Middle right lens centre (5th lens)
        centres[4, 0] = pitch_mla 
        centres[4, 1] = 0
        # Bottom left lens centre (6th lens)
        centres[5, 0] = - pitch_mla / 2
        centres[5, 1] = - pitch_mla * np.sqrt(3)/2
        # Bottom right lens centre (7th lens)
        centres[6, 0] = pitch_mla / 2
        centres[6, 1] = - pitch_mla * np.sqrt(3)/2 
...
def get_sag_value(R, r,mla_pitch):
    ...
    Standard formula: sag = sqrt(R^2 - r^2) - sqrt(R^2- (picth_mla/2)^2) 
...
#TODO: add another sag calculation using the 'long axis' as the picth. 

def get_sag_value_long_axis(R, r, mla_pitch):
    """
    Calculate the sag value for a spherical lens surface using the long axis.
    Standard formula: sag = sqrt(R^2 - r^2) - sqrt(R^2- (picth_mla/2)^2) 
    ...
    if np.any(valid_mask):
        ...
        # For convex surface (R > 0), this gives positive sag
        sqrt_term = np.sqrt(R_sq - r_sq[valid_mask])
        sag[valid_mask] = sqrt_term - np.sqrt(R_sq - (mla_pitch / np.sqrt(3))**2)  # 'long axis' as the pitch. 
...
def get_mla_sag_grid(mla_centres, mla_pitch, curvature, grid_size, grid_pixel_size,if_long_axis=False):
    """
    ...
    sag_values : np.ndarray
        2D array of sag values for the MLA on the grid.    
    """
    ...
    for centre in mla_centres: 
        ...
        # Calculate radial distance from the centre of the MLA
        radius_array = np.sqrt((x_grid - x_centre)**2 + (y_grid - y_centre)**2)
        
        # Create a mask for the MLA area (circular aperture)
        # Use mla_pitch/2 as the aperture radius for each lens
        mask = radius_array <= mla_pitch / 2
        
        # Calculate sag values for the MLA area
        if np.any(mask):
            if if_long_axis:
                current_sag = get_sag_value_long_axis(R=curvature, r=radius_array[mask], mla_pitch=mla_pitch)# Use the long axis sag calculation (overfilling)
            else:
                current_sag = get_sag_value(R=curvature, r=radius_array[mask], mla_pitch=mla_pitch) # Use the short axis sag calculation (underfilling)
            
            # Assign sag values to the sag grid
            # For overlapping areas, take maximum sag (not additive)
            sag_grid[mask] = np.maximum(sag_grid[mask], current_sag)
    
    return sag_grid

def get_mla_physical_mask(f_mla, n_material,mla_centres, pitch_mla, grid_size=(4e-3, 4e-3), grid_pixel_size=10e-6,if_long_axis=False):
    """
    Discontinued 
    """

    R = f_mla * (n_material - 1) # Radius of curvature of the MLA in (m). Assuming the MLA is a thin plano-convex lens. 

    # Calculate sag values for the MLA on a grid
    sag_values = get_mla_sag_grid(mla_centres=mla_centres, mla_pitch=pitch_mla, curvature=R, grid_size=grid_size, grid_pixel_size=grid_pixel_size,if_long_axis=if_long_axis)
    

    # Create a mask for the MLA area
    mask = np.abs(sag_values) > 0  # mask for the MLA area (non-zero sag values)
    # Create a grid of points in the XY plane
    x = np.arange(-grid_size[0]/2, grid_size[0]/2, grid_pixel_size) 
    y = np.arange(-grid_size[1]/2, grid_size[1]/2, grid_pixel_size)
    x_grid, y_grid = np.meshgrid(x, y)  # create a meshgrid of points in the XY plane
    # Get the coordinates of the MLA area
    x_mla = x_grid[mask]  # x coordinates of the MLA area
    y_mla = y_grid[mask]  # y coordinates of the MLA area
    sag_mla = sag_values[mask]  # sag values of the MLA area
    # Combine the coordinates and sag values into a single array
    mla_phycial_mask = np.column_stack((x_mla, y_mla, sag_mla))  # combine the coordinates and sag values into a single array

    return mla_phycial_mask # return the MLA design as an array of shape (N, 3) where N is the number of points in the MLA area

def generate_mla_design(f_fourier,f_mla,pitch_mla,lens_number,plot_design=False,if_long_axis=False):
    """
    Generate the MLA design based on the given parameters.
    
    Parameters:
    f_fourier : float
        Focal length of the Fourier lens in (m).
    f_mla : float
        Focal length of the MLA in (m).
    pitch_mla : float
        Pitch of the MLA in (m).
    
    Returns:
    mla_design : np.ndarray
        Array containing the MLA design coordinates.
    ...
    grid_side_length = np.ceil(bfp_diameter*1e3) / 1e3 # round up to the nearest mm for grid size
    current_grid_size = (grid_side_length, grid_side_length)  # size of the grid in (m) as (width, height)
    
    #calculate cuvature
    R = f_mla * (n_material - 1)
    ...
    sag_grid = get_mla_sag_grid(mla_centres=mla_centres, mla_pitch=pitch_mla, curvature=R, 
                               grid_size=current_grid_size, grid_pixel_size=sample_grid_size,if_long_axis=if_long_axis)

    # Convert sag values to micrometers
    ...
    if plot_design:
        ...
        ax.set_title(f'MLA Design: f_fourier={f_fourier*1e3:.1f}mm, f_mla={f_mla*1e3:.1f}mm, pitch_mla={pitch_mla*1e3:.1f}mm, lens_number={lens_number}')
        ...
        for centre in mla_centres:
            ...
            hexagon = RegularPolygon([x_pixel, y_pixel], numVertices=6, 
                                   radius=pitch_mla/(np.sqrt(3)*sample_grid_size), 
...
def generate_and_save_mla_designs(mla_params):
    ...
    """
    f_fourier, f_mla, pitch_mla, lens_number,plot_design,if_long_axis = mla_params
    mla_frame = generate_mla_design(f_fourier=f_fourier, f_mla=f_mla, pitch_mla=pitch_mla, lens_number=lens_number,plot_design=plot_design,if_long_axis=if_long_axis)
...
#Specify the MLA design parameters. As (9,6) array [f_fourier,f_mla, pitch_mla, lens_number, plot_design, if_long_axis]
mla_design_parameters = np.zeros((9, 6)) #initialize MLA design parameters array

mla_design_parameters[0] = [125e-3, 50e-3, 1.4e-3,3,True,False] # Array No. 1 : f_fourier - 125mm, f_mla - 50mm, pitch_mla - 1.4mm (3HEX)
mla_design_parameters[1] = [125e-3, 50e-3, 1.5e-3,3,True,False] # Array No. 2 : f_fourier - 125mm, f_mla - 50mm, pitch_mla - 1.5mm (3HEX)
mla_design_parameters[2] = [125e-3, 50e-3, 1.0e-3,7,True,False] # Array No. 3 : f_fourier - 125mm, f_mla - 50mm, pitch_mla - 1mm (7HEX)
mla_design_parameters[3] = [125e-3, 50e-3, 1.05e-3,7,True,False] # Array No. 4 : f_fourier - 125mm, f_mla - 50mm, pitch_mla - 1.05mm (7HEX)

#replicatng the firtst design for code debug.  #TODO: add more designs parameterrs later. 
mla_design_parameters[4] = [125e-3, 30e-3, 1.4e-3,3,True,True] # Array No. 5 : f_fourier - 125mm, f_mla - 30mm, pitch_mla - 1.4mm (3HEX) overfilled
mla_design_parameters[5] = [125e-3, 30e-3, 1.5e-3,3,True,True] # Array No. 6 : f_fourier - 125mm, f_mla - 30mm, pitch_mla - 1.5mm (3HEX) overfilled
mla_design_parameters[6] = [125e-3, 30e-3, 1.05e-3,7,True,True] # Array No. 7 : f_fourier - 125mm, f_mla - 30mm, pitch_mla - 1.05mm (7HEX) overfilled
mla_design_parameters[7] = [125e-3, 30e-3, 1.0e-3,7,True,True] # Array No. 8 : f_fourier - 125mm, f_mla - 30mm, pitch_mla - 1mm (7HEX) overfilled
mla_design_parameters[8] = [125e-3, 25e-3, 1.05e-3,7,True,True] # Array No. 9 : f_fourier - 125mm, f_mla - 25mm, pitch_mla - 1.05mm (7HEX) overfilled
...
# Visual check of the full MLA frame 
if is_plot_design:
    plt.figure(figsize=(10, 10))
    plt.imshow(full_mla_frame, cmap='YlOrRd', origin='lower', aspect='equal', interpolation='none')
    plt.title('Full MLA Design (15mm x 15mm)')
